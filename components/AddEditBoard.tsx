    ...
    <>
      ...
      <div className='w-[480px] min-h-[325px] flex flex-col p-8 rounded-md bg-white dark:bg-darkGrey absolute top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 z-20'>
        ...
        <div className='w-full h-[63px] flex flex-col justify-between my-6'>
          ...
          }} className='w-full h-[40px] px-4 py-2 text-bL dark:bg-darkGrey dark:text-white border border-mediumGrey/25 focus:border-purple focus:border-2 focus:px-[15px] outline-0 rounded' id="board_name" name='name' />
        </div>
        <div className='w-full flex flex-col mb-3'>
          <p className='text-hS text-mediumGrey dark:text-white mb-3'>Board Columns</p>
          {
            displayAddEditBoard.mode === 'ADD' && addBoardInputs.columns.map((column) => (
              <div className='w-full flex justify-between items-center last:mb-0 mb-3' key={column.id}>
                <input value={column.name} onChange={(e) => onChangeAddBoards(column.id, e.target.value, 'changeName')} className='w-[385px] h-[40px] px-4 py-2 text-bL dark:bg-darkGrey dark:text-white border border-mediumGrey/25 focus:border-purple focus:border-2 focus:px-[15px] outline-0 rounded' />
          ...
          }
          {
            displayAddEditBoard.mode === 'EDIT' && editBoardInputs.columns.map((column) => (
              <div className='w-full flex justify-between items-center last:mb-0 mb-3' key={column.id}>
                <input value={column.name} onChange={(e) => onChangeEditBoards(column.id, e.target.value, 'changeName')} className='w-[385px] h-[40px] px-4 py-2 text-bL dark:bg-darkGrey dark:text-white border border-mediumGrey/25 focus:border-purple focus:border-2 focus:px-[15px] outline-0 rounded' />
        ...
        }} type='button' className='w-full h-[40px] bg-purple/10 dark:bg-white hover:bg-purple/30 text-purple font-bold text-bL rounded-[20px]'>+ Add New Column</button>}
